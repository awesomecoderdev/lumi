...
<?php if ($product->get_attribute('pa_color')) : ?>
    ...
    <div class="relative flex flex-wrap gap-3 product-colors">
        <?php foreach ($colors as $key => $color) : ?>
            <div class="w-9 h-9 flex justify-center items-center cursor-pointer rounded-lg border product-colors-item <?php echo $key == 0 ? "active" : "" ?>" style="background: <?php echo get_lumi_product_color($color->term_id) ?>;">
            ...
            <!-- <span class="truncate text-center text-[10px]"><?php echo ucfirst($color->name); ?></span> -->
            <?php //clog($color)
            ?>
        <?php endforeach; ?>
    </div>
